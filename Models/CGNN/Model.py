...
class CrystalGraphConvNet(nn.Module):
    ...
    def __init__(self, orig_atom_fea_len, nbr_fea_len,
                 atom_fea_len=64, n_conv=3, h_fea_len=128, n_h=1,crystal_fea_len=15,
        ...
        self.conv_to_fc = nn.Linear(atom_fea_len + crystal_fea_len, h_fea_len)
        ...
        if self.classification:
            ...
            self.fc_out = nn.Linear(h_fea_len, 1) # For regression, there will only be 1 output unit.
        if self.classification:
            self.logsoftmax = nn.LogSoftmax(dim=1)
            self.dropout = nn.Dropout()

    def forward(self, atom_fea, nbr_fea, nbr_fea_idx, crystal_atom_idx, crystal_fea):
        """
        Forward pass
        N: Total number of atoms in the batch
        M: Max number of neighbors
        ...
        # Appending other additional features
        crys_fea = torch.cat((crys_fea,crystal_fea), dim = 1)
